...
class Histogram:
    ...
    def __init__(self, x, y, num_games, display_rate=False):
        self.x, self.y = x, y
        self.wins = [0, 0, 0, 0, 0, 0, 0]
        self.num_games = num_games
        self.start_time = time.time()
        self.display_rate = display_rate

    def report_win(self, guess_count):
        if guess_count is not None and 1 <= guess_count <= len(self.wins):
            self.wins[guess_count-1] += 1

    def draw(self, screen):
        pg.draw.rect(screen, "gray", (self.x, self.y, 100, 1))
        pg.draw.rect(screen, "gray", (self.x, self.y + 25, 100, 30))
        percent_complete = sum(self.wins)/self.num_games
        complete_color = "light gray"
        if percent_complete == 1:
            complete_color = "light green"
        pg.draw.rect(screen, complete_color, (self.x, self.y + 25, 100 * percent_complete, 30))
        for guess_count, num_wins in enumerate(self.wins):
            myfont = pg.font.SysFont("monospace", 15)
            text = myfont.render(str(guess_count+1), 1, "black")
            color = (121, 168, 107)
            if guess_count >= 6:
                text = myfont.render("L", 1, "maroon")
                color = "maroon"
            screen.blit(text, (self.x + 15 * guess_count, self.y + 10))
            bar_height = int(self.wins[guess_count] * 0.5)
        ...
        screen.blit(text, (self.x + 18, self.y + 30))
        if self.display_rate:
            myfont = pg.font.SysFont("monospace", 10)
            rate = sum(self.wins) / (time.time() - self.start_time)
            text = myfont.render(f"{rate:.1f} games/sec", 1, "black")
            screen.blit(text, (self.x + 50 - (text.get_width()/2), self.y + 60))
